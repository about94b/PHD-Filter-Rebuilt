function [ w_out,ksq,coef_list,tau_init ] = samAnalyticVel( we,Id,w_init,moi,time_list )
%SAMANALYTICVEL Analytically propogates the angular velocity of an object
%assuming no torque is being applied for long-axis mode rotations.
%   [ w_out,ksq,coef_list,tau_init ] = samAnalyticVel( w_init,moi,time_list )
%   Input:
...
% k-squared
ksq = (I1 - I3) * (I2 - Id) / ((I2 - I1) * (Id - I3));

% coefficients
coef_wx = we * sqrt(Id * (I2 - Id) / (I1 * (I2 - I1)));
if w_init(2) < 0
...
% w_test(2) = coef_wy * jacobiDN(tau_init,ksq);
% w_test(3) = coef_wz * jacobiCN(tau_init,ksq);

%% Propagate

% convert time_list to tau
tau_list = tau_init + we * ...
    sqrt(Id * (Id - I3) * (I2 - I1) / (I1 * I2 * I3)) * (time_list - time_list(1));

% individual components
wx_out = coef_wx * jacobiSN(tau_list,ksq);
wy_out = coef_wy * jacobiDN(tau_list,ksq);
wz_out = coef_wz * jacobiCN(tau_list,ksq);
